...
async function send (cmd, comment = null, responseMaxLength = 40) {
  const b = Buffer.from(cmd);

  console.log((comment ? `[${comment}] ` : "") + `sending`, b);

  const data = await readerProxy.transmit(b, responseMaxLength);
...
ipcMain.on('write-info', (event, message) => {
  writeData(message);
})

// Handle an asynchronous request
ipcMain.handle('operation-read', async (event, arg) => {
  try {
      // Perform an async operation here, like fetching data or reading from a file
      const result = await readData();
      return result;  // This will be sent back to the renderer process
  } catch (error) {
      // Handle error (e.g., throw or return a specific error message)
